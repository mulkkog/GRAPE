...
@register('grape-enc-gs')
class GRAPE(nn.Module): 
    def __init__(self, encoder_spec, feat_dim: int = 256, ensemble_size: int = 4, **kwargs):
        super().__init__()

        # ─────────────────────── encoder 그대로
        self.encoder = models.make(encoder_spec)
 
        # 3) Gaussian head = 1×1 Conv
        self.feat_dim = feat_dim
        self.gauss_feat_dim = feat_dim + 5  # θ(1)+scale(2)+offset(2)+feat(D)
        self.head_in_channels = 256  # 64×4 after PixelUnshuffle
        self.gauss_feat_head = nn.Conv2d(self.head_in_channels, self.gauss_feat_dim, 1)
 
        self.gauss_rgb_dim = 8  # θ(1)+scale(2)+offset(2)+rgb(3) 
        self.gauss_rgb_head = nn.Conv2d(64, self.gauss_rgb_dim, 1)

        self.ps = nn.PixelShuffle(2)

        # 4) HR latent → RGB
        self.to_rgb = nn.Conv2d(self.feat_dim, 3, 1)

        # background vector (D‑dim)
        self.register_buffer("feat_background", torch.zeros(feat_dim))
        self.register_buffer("rgb_background", torch.zeros(3))


    def get_coord(self, width, height, device):
        x = torch.arange(width,  device=device)
        y = torch.arange(height, device=device)
        xg, yg = torch.meshgrid(x, y, indexing="ij")
        xg = 2 * (xg / width)  - 1
    ...
    def gen_feat(self, inp):
        self.inp = inp
        self.feat = self.encoder(inp)           # (B,64,H,W) 
        return self.feat
    ...
    def query_feat(self):
        ...
        if self.scale1 > 1.0:
            coord = self.get_coord(h_inp * 2, w_inp * 2, self.inp.device).unsqueeze(0).expand(B, -1, 2)
            pred_map = self.gauss_rgb_head(self.feat)  # (B,gauss_dim,H/2,W/2)
            pred = pred_map.view(B, self.gauss_rgb_dim, -1).permute(0, 2, 1).contiguous()
            feat_vec   = pred[..., 0:3]                  # (B,N,3)
            theta_raw  = pred[..., 3:4]                  # (B,N,1)
            scale_raw  = pred[..., 4:6]                  # (B,N,2)
            offset_raw = pred[..., 6:8]                  # (B,N,2)
 
        else:
            coord = self.get_coord(h_inp, w_inp, self.inp.device).unsqueeze(0).expand(B, -1, 2)
            pred_map = self.gauss_feat_head(self.feat)  # (B,gauss_dim,H/2,W/2)
            pred = pred_map.view(B, self.gauss_feat_dim, -1).permute(0, 2, 1).contiguous()
            feat_vec = pred[..., : self.feat_dim]
            theta_raw, scale_raw, offset_raw = torch.split(pred[..., self.feat_dim : ], [1, 2, 2], dim=-1)

        theta = torch.sigmoid(theta_raw) * 2 * math.pi
        scale_xy = torch.sigmoid(scale_raw) * 0.5
        scale_xy = torch.stack([scale_xy[..., 0] * self.scale1, scale_xy[..., 1] * self.scale2], dim=-1) + eps
        offset = torch.tanh(offset_raw)
        ...
        hr_feat = self.rasterize(feat_vec, scale_xy, theta, xyz, B)
        return hr_feat
    ...
    def forward(self, inp, scale):
        ...
        self.gen_feat(inp)
        hr_feat = self.ps(self.query_feat())
    ...
    def rasterize(self, sampled_feat, scale_out, rot_out, xyz, B):
        ...
        if sampled_feat.shape[-1] == 3:
            background = self.rgb_background
        else:
            background = self.feat_background
        rendered = []
        for i in range(B):
            xys, depths, radii, conics, num_hits = project_gaussians_2d_scale_rot(
                xyz[i], scale_out[i], rot_out[i],
            ...
            if out_img.shape[0] == 3:
                out_img = out_img.clamp_(0.0, 1.0)   # (3,H,W)
            rendered.append(out_img)

        return torch.stack(rendered, dim=0)                        # (B,3,H,W)
 

@register('grape-enc-gs2')
class GRAPE(nn.Module): 
    def __init__(self, encoder_spec, feat_dim: int = 256, ensemble_size: int = 4, **kwargs):
        super().__init__()
        ...
        self.gauss_feat_dim = feat_dim + 5  # θ(1)+scale(2)+offset(2)+feat(D)
        self.head_in_channels = 256  # 64×4 after PixelUnshuffle
        self.gauss_feat_head = nn.Conv2d(3, self.gauss_feat_dim, 1)
...
class GRAPE(nn.Module): 
    def __init__(self, encoder_spec, feat_dim: int = 256, ensemble_size: int = 4, **kwargs):
        ...
        self.gauss_feat_dim = feat_dim + 5  # θ(1)+scale(2)+offset(2)+feat(D)
        self.head_in_channels = 256  # 64×4 after PixelUnshuffle
        self.gauss_feat_head_1 = nn.Conv2d(3, self.gauss_feat_dim, 1)
        self.gauss_feat_head_2 = nn.Conv2d(64, self.gauss_feat_dim, 1)
 
        self.gauss_rgb_dim = 8  # θ(1)+scale(2)+offset(2)+rgb(3) 
        self.gauss_rgb_head = nn.Conv2d(256, self.gauss_rgb_dim, 1)
    ...
    # ------------------------------------------------------------------
    def query_feat(self):
        B, _, h_inp, w_inp = self.feat.shape
        eps = 1e-6
        self.H = round(h_inp * self.scale1)
        self.W = round(w_inp * self.scale2)

        coord = self.get_coord(h_inp, w_inp , self.feat.device).unsqueeze(0).expand(B, -1, 2)
        ...
        if self.scale1 > 2.0:
        ...
        elif self.scale1 == 2.0:
            # coord = self.get_coord(h_inp , w_inp  , self.feat.device).unsqueeze(0).expand(B, -1, 2)
            pred_map = self.gauss_feat_head_2(self.feat) 
            pred = pred_map.view(B, self.gauss_feat_dim, -1).permute(0, 2, 1).contiguous()
            feat_vec = pred[..., : self.feat_dim]
            theta_raw, scale_raw, offset_raw = torch.split(pred[..., self.feat_dim : ], [1, 2, 2], dim=-1)
            ...
            # coord = self.get_coord(h_inp, w_inp, self.feat.device).unsqueeze(0).expand(B, -1, 2)
            pred_map = self.gauss_feat_head_1(self.feat) 
            pred = pred_map.view(B, self.gauss_feat_dim, -1).permute(0, 2, 1).contiguous()
            feat_vec = pred[..., : self.feat_dim]
            theta_raw, scale_raw, offset_raw = torch.split(pred[..., self.feat_dim : ], [1, 2, 2], dim=-1)
    ...
    def forward(self, inp, scale):
        ...
        self.step = 2
        self.feat = hr_feat
        self.scale1 = self.scale2 = 2.0
        hr_feat = self.query_feat() 
...
@register('gs-enc-baseline')
class GaussFeatureEncoder(nn.Module):
    """A *tiny* CNN that outputs a 64‑channel feature map at native resolution."""

    def __init__(self, in_ch = 3, base = 32, n_feats=256):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(in_ch, base, 3, 1, 1), nn.ReLU(inplace=True),
            nn.Conv2d(base, base, 3, 1, 1), nn.ReLU(inplace=True),
            nn.Conv2d(base, base * 2, 3, 1, 1), nn.ReLU(inplace=True),  # 2× width
            nn.Conv2d(base * 2, n_feats, 3, 1, 1), nn.ReLU(inplace=True),
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.net(x)  # (B,64,H,W)