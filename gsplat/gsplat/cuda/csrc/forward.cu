...
__global__ void rasterize_forward_sum(
    const dim3   tile_bounds,              // how many tiles in (x, y)
    const dim3   img_size,                 // full image (W, H)
    const int32_t* __restrict__ gaussian_ids_sorted,
    const int2*   __restrict__ tile_bins,  // per‑tile [start, end) indices into gaussian_ids_sorted
    const float2* __restrict__ xys,        // gaussian centers (x, y)
    const float3* __restrict__ conics,     // quadratic form parameters (a, b, c)
    const float3* __restrict__ colors,     // RGB colors  (0‑1)
    const float*  __restrict__ opacities,  // per‑gaussian opacity (0‑1)
    float*        __restrict__ final_Ts,   // [H*W]  remaining transmittance per pixel
    int*          __restrict__ final_index,// [H*W]  last gaussian idx per pixel (within its tile bin)
    float3*       __restrict__ out_img,    // [H*W]  output HDR color
    ...
    unsigned i = block.group_index().y * block.group_dim().y + block.thread_index().y; // pixel y
    unsigned j = block.group_index().x * block.group_dim().x + block.thread_index().x; // pixel x

    float px = static_cast<float>(j);
    float py = static_cast<float>(i);
    int32_t pix_id = i * img_size.x + j;

    bool inside = (i < img_size.y && j < img_size.x);

    // -------------------------------------------------------------------------
    // fetch gaussian range for this tile & batching info
    // -------------------------------------------------------------------------
    int2 range = tile_bins[tile_id];
    int num_batches = (range.y - range.x + BLOCK_SIZE - 1) / BLOCK_SIZE;
    ...
    float  T        = 1.f;   // remaining visibility through all processed gaussians
    int    cur_idx  = -1;    // last contributing gaussian (relative to tile_bins)
    float3 pix_out  = {0.f, 0.f, 0.f};

    int tr = block.thread_rank();

    bool done = !inside;     // outside pixels are done from the start

    // -------------------------------------------------------------------------
    // iterate over batches of at most 256 gaussians
    // -------------------------------------------------------------------------
    for (int b = 0; b < num_batches; ++b) {
        int batch_start = range.x + BLOCK_SIZE * b;
        ...
        block.sync(); // make sure shared memory is fully populated

        int batch_size = min(BLOCK_SIZE, range.y - batch_start);

        // per‑pixel rasterization over the gaussians in this batch
        for (int t = 0; t < batch_size && !done; ++t) {
            const float3 conic    = conic_batch[t];
            const float3 xy_opac  = xy_opacity_batch[t];
            ...
            if (sigma < 0.f || alpha < (1.f / 255.f))
                continue; // gaussian contributes negligibly

            const float vis = alpha;          // visibility (= opacity) for additive comp
            const float3 c  = colors[id_batch[t]];

            pix_out.x += c.x * vis;
            pix_out.y += c.y * vis;
            pix_out.z += c.z * vis;

            //T *= (1.f - vis);                // update remaining transmittance
            cur_idx = batch_start + t;

            //if (T < (1.f / 255.f))           // pixel saturated → we can stop early
            //    done = true;
        }

        block.sync(); // ensure everyone updated `done` before tile‑wide check

        if (__syncthreads_count(done) == BLOCK_SIZE)
            break;  // every thread in tile is done → break out of batch loop
    }
