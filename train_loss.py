...
def main(config_, save_path):
    ...
    for epoch in range(start_epoch, epoch_max + 1):
        ...
        for batch in tqdm(train_loader, leave=False, desc=f'Epoch {epoch}/{epoch_max}'):
            ...
            # ──────────────────────────────── Forward
            pred = model(inp, batch['scale'])     # [B, 3, H, W]
            params = model.module._last_params if n_gpus > 1 else model._last_params

            # ──────────────────────────────── GT RGB @ predicted offset 위치
            offset = params['offset'].detach()                  # [B, N, 2], normalized [-1, 1]
            offset_grid = offset.unsqueeze(2)                   # [B, N, 1, 2]
            gt_rgb_sampled = F.grid_sample(
                gt, offset_grid, mode='bilinear', align_corners=True
            )  # [B, 3, N, 1]
            gt_rgb_sampled = gt_rgb_sampled.squeeze(-1).permute(0, 2, 1)  # [B, N, 3]
            loss_rgb = F.l1_loss(params['rgb'], gt_rgb_sampled)
 
            # ──────────────────────────────── 전체 Loss
            loss_l1 = l1_fn(pred, gt)
            loss = (
                loss_l1 +
                0.5 * loss_rgb 
            )
